...
// Middlewares
app.use(cors());
app.use(express.json()); // JSON parsing between requests bodies
...
app.post('/api/login', async (req, res) => {
  ...
  if (!email || !senha) {
    return res.status(400).json({ message: 'No e-mail or psswd provided bro!' });
  }
  ...
  try {
    ...
    const query = 'select cod_usuario, nome, email, nivel from usuarios where email = ? AND senha = ?';
...
app.get('/api/incidentes', async (req, res) => {
  ...
  try {
    ...
    if (atendenteId) {
      ...
      query += ' where incidentes.atendente is null or incidentes.atendente = 0';
...
app.get('/api/atendentes', async (req, res) => {
  ...
  try {
    ...
    const query = "select cod_usuario, nome from usuarios where nivel = 'Atendente' or nivel = 'Administrador'";
...
app.post('/api/incidentes', async (req, res) => {
  ...
  if (!usuario || !assunto || !descricao) {
    return res.status(400).json({ message: 'Usuário, assunto e descrição are needed.' });
  }
  ...
  try {
    ...
    const params = [usuario, atendente || null, assunto, descricao, data_previsao || null];
...
app.put('/api/incidentes/:id', async (req, res) => {
  ...
  try {
    conn = await dbPool.getConnection();
    const query = 'update incidentes set atendente = ?, data_previsao = ? where cod_incidente = ?';
